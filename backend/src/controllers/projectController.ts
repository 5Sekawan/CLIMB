...
export class ProjectController {
   ...
   * Update project status (active, finished, inactive).
   */
  static async updateProjectStatus(req: Request, res: Response) {
    try {
      ...
      if (!['active', 'finished', 'inactive'].includes(status)) {
      ...
      await ActivityService.log('system', `Updated project status to ${status}`, project.id, project.name);
